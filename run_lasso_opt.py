...
import os
import sys
from compute_risk import comp_risk_ridge, compute_risk_lasso
...
path_result = 'result/lasso/opt/{:.02f}_{:.02f}/'.format(sparse, rho)
...
def run_ridge(phi, psi, lam, sparse, sigma):
    risk_list = []

    try:
        tau2, xi2 = comp_risk_ridge(
            rho, sigma, lam, phi, psi)        
    except:
        tau2, xi2 = np.nan, np.nan
        print(phi,lam)
    risk_list.append([phi, psi, lam, sparse, 1., tau2, xi2])
    risk_list = np.array(risk_list)
    return risk_list
...
with Parallel(n_jobs=16, verbose=12, timeout=99999) as parallel:
    ...
    for phi in tqdm(phi_list, desc='phi'):
        ...
        res = parallel(delayed(run_ridge)(
            phi, psi, lam, sparse=sparse, sigma=sigma) for psi in tqdm(psi_list, desc='psi'))
        
        res = np.concatenate(res, axis=0)
        df = pd.DataFrame(res, columns=['phi', 'psi', 'lam', 'sparse', 'sigma', 'R1', 'Rinf'])
        df.to_pickle(path_result+'res_ridgeless_{:.02f}.pkl'.format(phi), compression='gzip')        
