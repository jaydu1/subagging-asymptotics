...
def run_one_simulation(phi, psi, lam, M, i, cov='normal'):
    p = int(n*phi)
    ...
    if psi<phi:
        ...
        _, _, X, Y, X_test, Y_test, _, _ = generate_data(
            p, phi, 0., sigma, rho=rho, coef=coef, 
            cov=cov, func='linear', n_test=1000)
...
with Parallel(n_jobs=8, verbose=0, timeout=99999) as parallel:            
    ...
    for cov in ['rademacher', 'uniform']: #  't-20',]:#
        res = parallel(
            delayed(run_one_simulation)(phi, psi, lam, M, i, cov)
                for psi in tqdm(psi_list, desc='psi') for lam in tqdm(lam_list, desc='lam') for i in tqdm(np.arange(n_simu), desc='i')
        )
        df = pd.DataFrame(np.concatenate(res, axis=0), columns=['phi','psi','lam','i','M','risk','cov'])
        df.to_pickle(path_result+'res_{}_est_phi_{:.01f}.pkl'.format(cov, phi), compression='gzip')
